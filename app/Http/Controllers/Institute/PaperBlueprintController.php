...
namespace App\Http\Controllers\Institute;
    ...
    class PaperBlueprintController extends Controller
    {
        ...
        public function storeRule(Request $request, BlueprintSection $section)
        {
            ...
            return redirect()->route('institute.blueprints.show', $section->paperBlueprint)
        ...
        {
            // Load the correct relations based on your models/migrations
            $rule->loadMissing('blueprintSection.paperBlueprint');

            // Guard against orphaned data / mis-linked rows
            if (!$rule->blueprintSection || !$rule->blueprintSection->paperBlueprint) {
                abort(404, 'Parent section or blueprint not found for this rule.');
            }

            // Ownership check
            if ((int) $rule->blueprintSection->paperBlueprint->institute_id !== (int) auth()->id()) {
                abort(403);
            }

            $blueprint = $rule->blueprintSection->paperBlueprint;
            $sectionName = $rule->blueprintSection->name;

            $rule->delete();

            return redirect()
                ->route('institute.blueprints.show', $blueprint)
                ->with('success', "Rule deleted from {$sectionName}.");
        }



        public function destroySection(BlueprintSection $section)
        {
            // Security: ensure the logged-in institute owns the parent blueprint
            if ($section->paperBlueprint->institute_id !== auth()->id()) {
                abort(403);
            }

            $blueprint = $section->paperBlueprint;
            $sectionName = $section->name;

            // If your FK constraints are set to cascade, the next line alone is enough.
            // Calling delete on rules first is safe if you don't have cascade deletes.
            $section->rules()->delete();    // safe-guard if no ON DELETE CASCADE
            $section->delete();
