<x-app-layout>
                ...
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    <form action="{{ route('admin.questions.update', $question) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="mt-4">
                                <label for="question_image" class="block text-sm font-medium text-gray-700">New Question Diagram (Optional)</label>
                                ...
                                @if($question->question_image_path)
                                    <div class="mt-2 text-sm text-gray-500">Current Image: <img id="question_image_preview" src="{{ asset('storage/' . $question->question_image_path) }}" class="inline-block h-16 w-auto border p-1 rounded"></div>
                                @else
                                    <img id="question_image_preview" src="#" alt="New Image Preview" class="mt-2 rounded-md max-h-48 border p-1" style="display: none;"/>
                                @endif
                            </div>
                        ...
                        <div>
                            ...
                            <div class="mt-4">
                                <label for="answer_image" class="block text-sm font-medium text-gray-700">New Answer Diagram (Optional)</label>
                                ...
                                @if($question->answer_image_path)
                                    <div class="mt-2 text-sm text-gray-500">Current Image: <img id="answer_image_preview" src="{{ asset('storage/' . $question->answer_image_path) }}" class="inline-block h-16 w-auto border p-1 rounded"></div>
                                @else
                                     <img id="answer_image_preview" src="#" alt="New Image Preview" class="mt-2 rounded-md max-h-48 border p-1" style="display: none;"/>
                                @endif
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // --- MCQ Options & Image Preview Logic ---
            ...
            addOptionBtn.addEventListener('click', function () {
                const optionCount = optionsWrapper.querySelectorAll('input[type="text"]').length;
            ...
            const answerImageInput = document.getElementById('answer_image');
            const answerImagePreview = document.getElementById('answer_image_preview');
            ...
            questionImageInput.addEventListener('change', function(event) {
                if (event.target.files && event.target.files[0]) {
                    ...
                    reader.onload = function(e) {
                        ...
                        questionImagePreview.style.display = 'inline-block';
                    }
                    reader.readAsDataURL(event.target.files[0]);
                }
            });

            answerImageInput.addEventListener('change', function(event) {
                if (event.target.files && event.target.files[0]) {
                    const reader = new FileReader();
                    reader.onload = function(e) {
                        answerImagePreview.src = e.target.result;
                        answerImagePreview.style.display = 'inline-block';
                    }
                    reader.readAsDataURL(event.target.files[0]);
                }
            });
